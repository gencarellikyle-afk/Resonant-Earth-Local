...
function PasswordGate({onUnlock}){const[pw,setPw]=useState("");const[err,setErr]=useState(false);const[shake,setShake]=useState(false);const[locked,setLocked]=useState(false);const[lockTimer,setLockTimer]=useState(0);const attempts=useRef(0);function submit(){if(locked)return;if(pw===process.env.NEXT_PUBLIC_ECO_PW){sessionStorage.setItem("eco_auth","1");onUnlock();}else{attempts.current++;setErr(true);setShake(true);setTimeout(()=>setShake(false),600);if(attempts.current>=5){setLocked(true);let t=60;setLockTimer(t);const id=setInterval(()=>{t--;setLockTimer(t);if(t<=0){clearInterval(id);setLocked(false);attempts.current=0;}},1000);}setTimeout(()=>setErr(false),2000);}}
...
function StatTile({label,value,coherence}){return(<div style={{flex:1,minWidth:"140px",padding:"1.2rem",border:"1px solid rgba(232,221,208,0.08)",borderLeft:"2px solid rgba(193,68,14,0.5)",backgroundColor:"rgba(232,221,208,0.02)"}}>
...
function GrantCard({grant,password,onUpdate}){const[approveAmt,setApproveAmt]=useState(grant.amount_requested||"");const[actStatus,setActStatus]=useState("idle");const[approveStep,setApproveStep]=useState(1);const[approveConfirmAmt,setApproveConfirmAmt]=useState("");const[approveNote,setApproveNote]=useState("");const[triageText,setTriageText]=useState("");const[triageStatus,setTriageStatus]=useState("idle");const[expanded,setExpanded]=useState(false);const typeColor=grant.grant_type==="creation"?"rgba(222,188,75,0.8)":grant.grant_type==="relief"?"rgba(193,68,14,0.8)":"rgba(60,160,120,0.8)";
async function act(action){setActStatus("sending");try{const r=await fetch("/api/eco/grant",{method:"POST",headers:{"Content-Type":"application/json","x-eco-password":password},body:JSON.stringify({id:grant.id,action,amount_approved:parseFloat(approveAmt),notes:(grant.notes||"")+" | APPROVAL NOTE: "+approveNote})});const d=await r.json();if(d.error)throw new Error(d.error);setActStatus("idle");setApproveStep(1);setApproveConfirmAmt("");setApproveNote("");onUpdate();}catch(e){setActStatus("error");setTimeout(()=>setActStatus("idle"),3000);}}
async function runTriage(){setTriageStatus("loading");setExpanded(true);try{const r=await fetch("/api/eco/agent",{method:"POST",headers:{"Content-Type":"application/json","x-eco-password":password},body:JSON.stringify({type:"triage",data:{grant_type:grant.grant_type,amount_requested:grant.amount_requested,description:grant.description,intended_use:grant.intended_use,coherence_threshold_met:grant.coherence_threshold_met,signal_count:grant.notes?.match(/(d+) signal/)?.[1]||"0",has_contact:!!(grant.contact),contact_method:grant.contact||null,wants_updates:grant.wants_updates,payment_target:grant.payment_target||null,payment_recipient:grant.payment_recipient||null,payment_deadline:grant.payment_deadline||null,ref:grant.notes?.match(/Ref: ([A-Z0-9-]+)/)?.[1]||""}})});const d=await r.json();setTriageText(d.text||"Triage unavailable.");setTriageStatus("done");}catch(e){setTriageText("Agent error. Review manually.");setTriageStatus("done");}}
const notesForDisplay=grant.notes?grant.notes.replace(/Contact:.*?|/,"").replace(/Signal presence:.*?|/,"").replace(/No contact.*?|/,"").replace(/No prior.*?|/,"").replace(/Ref:.*$/,"").trim():"";
return(<div style={{padding:"1.2rem",border:"1px solid rgba(232,221,208,0.08)",borderLeft:"2px solid "+typeColor,backgroundColor:"rgba(232,221,208,0.02)",display:"flex",flexDirection:"column",gap:"0.6rem",marginBottom:"0.75rem"}}>
...
{grant.notes?.match(/Ref: ([A-Z0-9-]+)/)?.[1]&&<p style={{fontFamily:"monospace",fontSize:"0.52rem",color:"rgba(232,221,208,0.2)",letterSpacing:"0.08em"}}>{grant.notes.match(/Ref: ([A-Z0-9-]+)/)[1]}</p>}
</div>
</div>
<p style={{fontFamily:"monospace",fontSize:"0.62rem",color:"rgba(232,221,208,0.4)"}}>Field ID: {grant.applicant_anonymous_id||"anonymous"}</p>
{grant.email&&<p style={{fontFamily:"monospace",fontSize:"0.65rem",color:"rgba(193,68,14,0.8)"}}>? {grant.email}</p>}
<div style={{display:"flex",gap:"1rem",alignItems:"center",flexWrap:"wrap"}}>
<p style={{fontFamily:"monospace",fontSize:"0.65rem",color:"rgba(232,221,208,0.7)",fontWeight:"bold"}}>Requested: {"$"+grant.amount_requested?.toLocaleString()}</p>
<p style={{fontFamily:"monospace",fontSize:"0.62rem",color:grant.coherence_threshold_met?"rgba(212,160,60,0.8)":"rgba(232,221,208,0.3)"}}>Coherence: {grant.coherence_threshold_met?"MET":"NOT MET"}</p>
{grant.notes?.match(/(d+) signal/)?.[1]&&<p style={{fontFamily:"monospace",fontSize:"0.58rem",color:"rgba(232,221,208,0.35)"}}>Signals: {grant.notes.match(/(d+) signal/)[1]}</p>}
</div>
{grant.description&&<div style={{borderTop:"1px solid rgba(232,221,208,0.06)",paddingTop:"0.6rem"}}>
<p style={{fontFamily:"monospace",fontSize:"0.52rem",letterSpacing:"0.18em",textTransform:"uppercase",color:"rgba(232,221,208,0.25)",marginBottom:"0.35rem"}}>What they are building</p>
<p style={{fontFamily:"Georgia,serif",fontStyle:"italic",fontSize:"0.82rem",color:"rgba(232,221,208,0.65)",lineHeight:1.75}}>{grant.description}</p>
</div>}
{grant.intended_use&&<div style={{borderTop:"1px solid rgba(232,221,208,0.06)",paddingTop:"0.6rem"}}>
<p style={{fontFamily:"monospace",fontSize:"0.52rem",letterSpacing:"0.18em",textTransform:"uppercase",color:"rgba(232,221,208,0.25)",marginBottom:"0.35rem"}}>How they will use it</p>
<p style={{fontFamily:"Georgia,serif",fontStyle:"italic",fontSize:"0.82rem",color:"rgba(232,221,208,0.65)",lineHeight:1.75}}>{grant.intended_use}</p>
</div>}
<div style={{borderTop:"1px solid rgba(232,221,208,0.06)",paddingTop:"0.75rem",display:"flex",flexDirection:"column",gap:"0.6rem"}}>
<button onClick={runTriage} disabled={triageStatus==="loading"} style={{alignSelf:"flex-start",padding:"0.4rem 1rem",border:"1px solid rgba(212,160,60,0.4)",backgroundColor:triageStatus==="done"?"rgba(212,160,60,0.05)":"transparent",color:"rgba(212,160,60,0.8)",fontFamily:"monospace",fontSize:"0.58rem",letterSpacing:"0.15em",textTransform:"uppercase",cursor:"crosshair",transition:"all 0.2s"}}>
{triageStatus==="loading"?"Analyzing...":triageStatus==="done"?"Re-Analyze":"Analyze Application"}</button>
{expanded&&triageText&&<div style={{padding:"1rem",backgroundColor:"rgba(212,160,60,0.04)",border:"1px solid rgba(212,160,60,0.15)",borderRadius:"2px"}}>
<p style={{fontFamily:"monospace",fontSize:"0.52rem",letterSpacing:"0.18em",textTransform:"uppercase",color:"rgba(212,160,60,0.5)",marginBottom:"0.5rem"}}>AI Triage Brief</p>
<p style={{fontFamily:"Georgia,serif",fontStyle:"italic",fontSize:"0.82rem",color:"rgba(232,221,208,0.75)",lineHeight:1.8,whiteSpace:"pre-wrap"}}>{triageText}</p>
</div>}
</div>
{grant.status==="pending"&&<div style={{borderTop:"1px solid rgba(232,221,208,0.06)",paddingTop:"0.75rem",display:"flex",flexDirection:"column",gap:"0.75rem"}}>
{approveStep===1&&<div style={{display:"flex",gap:"0.75rem",alignItems:"center",flexWrap:"wrap"}}>
...
<button onClick={()=>{if(!approveAmt)return;setApproveStep(2);}} style={{padding:"0.4rem 1rem",border:"1px solid rgba(212,160,60,0.5)",backgroundColor:"transparent",color:"rgba(212,160,60,0.8)",fontFamily:"monospace",fontSize:"0.6rem",letterSpacing:"0.15em",textTransform:"uppercase",cursor:"crosshair",transition:"all 0.3s ease"}}>Approve</button>
<button onClick={()=>act("decline")} disabled={actStatus==="sending"} style={{padding:"0.4rem 1rem",border:"1px solid rgba(193,68,14,0.35)",backgroundColor:"transparent",color:"rgba(193,68,14,0.6)",fontFamily:"monospace",fontSize:"0.6rem",letterSpacing:"0.15em",textTransform:"uppercase",cursor:"crosshair",transition:"all 0.3s ease"}}>Decline</button>
</div>}
{approveStep===2&&<div style={{display:"flex",flexDirection:"column",gap:"0.6rem",padding:"1rem",border:"1px solid rgba(212,160,60,0.25)",backgroundColor:"rgba(212,160,60,0.04)",borderRadius:"2px"}}>
<p style={{fontFamily:"monospace",fontSize:"0.58rem",color:"rgba(212,160,60,0.8)",letterSpacing:"0.15em",textTransform:"uppercase"}}>Confirm Approval � {"$"+parseFloat(approveAmt).toLocaleString()}</p>
<p style={{fontFamily:"monospace",fontSize:"0.6rem",color:"rgba(232,221,208,0.45)"}}>Type the amount below to confirm. This cannot be undone.</p>
<input type="number" value={approveConfirmAmt} onChange={e=>setApproveConfirmAmt(e.target.value)} placeholder={"Type "+approveAmt+" to confirm"} style={{padding:"0.4rem 0.6rem",backgroundColor:"rgba(232,221,208,0.04)",border:"1px solid "+(approveConfirmAmt===approveAmt?"rgba(212,160,60,0.6)":"rgba(232,221,208,0.12)"),outline:"none",fontFamily:"monospace",fontSize:"0.65rem",color:"rgba(232,221,208,0.8)",width:"160px",cursor:"crosshair"}}/>
<input type="text" value={approveNote} onChange={e=>setApproveNote(e.target.value)} placeholder="Why approved � required" style={{padding:"0.4rem 0.6rem",backgroundColor:"rgba(232,221,208,0.04)",border:"1px solid rgba(232,221,208,0.12)",outline:"none",fontFamily:"monospace",fontSize:"0.65rem",color:"rgba(232,221,208,0.8)",cursor:"crosshair"}}/>
<div style={{display:"flex",gap:"0.75rem"}}>
<button onClick={()=>act("approve")} disabled={actStatus==="sending"||approveConfirmAmt!==approveAmt||!approveNote.trim()} style={{padding:"0.4rem 1rem",border:"1px solid rgba(212,160,60,0.5)",backgroundColor:approveConfirmAmt===approveAmt&&approveNote.trim()?"rgba(212,160,60,0.1)":"transparent",color:approveConfirmAmt===approveAmt&&approveNote.trim()?"rgba(212,160,60,0.9)":"rgba(232,221,208,0.2)",fontFamily:"monospace",fontSize:"0.6rem",letterSpacing:"0.15em",textTransform:"uppercase",cursor:approveConfirmAmt===approveAmt&&approveNote.trim()?"crosshair":"default",transition:"all 0.2s"}}>{"Confirm � Release $"+parseFloat(approveAmt||"0").toLocaleString()}</button>
<button onClick={()=>{setApproveStep(1);setApproveConfirmAmt("");setApproveNote("");}} style={{padding:"0.4rem 1rem",border:"1px solid rgba(232,221,208,0.1)",backgroundColor:"transparent",color:"rgba(232,221,208,0.35)",fontFamily:"monospace",fontSize:"0.6rem",letterSpacing:"0.15em",textTransform:"uppercase",cursor:"crosshair"}}>Cancel</button>
</div>
{actStatus==="error"&&<p style={{fontFamily:"monospace",fontSize:"0.58rem",color:"rgba(193,68,14,0.8)"}}>Error. Try again.</p>}
</div>}
</div>}
{grant.status!=="pending"&&<p style={{fontFamily:"monospace",fontSize:"0.58rem",color:grant.status==="approved"?"rgba(212,160,60,0.7)":"rgba(232,221,208,0.3)",letterSpacing:"0.15em",textTransform:"uppercase",borderTop:"1px solid rgba(232,221,208,0.06)",paddingTop:"0.6rem"}}>{grant.status}{grant.status==="approved"&&grant.amount_approved?" � $"+grant.amount_approved?.toLocaleString():""}</p>}
</div>);}
...
async function loadData(){const today=new Date().toISOString().split("T")[0];const[sigAll,sigToday,waitlist,sweeps,grantsData,recentSigs]=await Promise.all([supabase.from("signals").select("id,country").limit(1000),supabase.from("signals").select("id").gte("created_at",today+"T00:00:00").limit(1000),supabase.from("waitlist").select("id").limit(1000),supabase.from("sweep_events").select("*").order("created_at",{ascending:false}),supabase.from("grants").select("*").order("created_at",{ascending:false}),supabase.from("signals").select("id,text,coherence,country,created_at").order("created_at",{ascending:false}).limit(20)]);const countries=new Set((sigAll.data||[]).map(s=>s.country).filter(Boolean)).size;const balance=sweeps.data&&sweeps.data.length>0?sweeps.data[0].grant_pool_balance:0;const totalSwept=(sweeps.data||[]).reduce((a,s)=>a+s.sweep_amount,0);const totalGranted=(grantsData.data||[]).filter(g=>g.status==="approved").reduce((a,g)=>a+(g.amount_approved||0),0);setStats({signals:(sigAll.data||[]).length,signalsToday:(sigToday.data||[]).length,waitlist:(waitlist.data||[]).length,countries});setPool({balance,totalSwept,totalGranted});setSweepEvents(sweeps.data||[]);setGrants(grantsData.data||[]);setSignals(recentSigs.data||[]);}
...
{Object.entries(signals.reduce((acc,s)=>{if(s.country){acc[s.country]=(acc[s.country]||0)+1;}return acc;},{} as Record<string,number>)).sort((a,b)=>(b[1] as number)-(a[1] as number)).slice(0,8).map(([country,count])=>(<div key={country} style={{display:"flex",gap:"0.75rem",alignItems:"center",marginBottom:"0.4rem"}}>
