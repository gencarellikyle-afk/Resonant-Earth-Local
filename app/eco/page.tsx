...
async function loadData(){const today=new Date().toISOString().split("T")[0];const[sigAll,sigToday,waitlist,sweeps,grantsData,recentSigs]=await Promise.all([supabase.from("signals").select("*",{count:"exact",head:false}),supabase.from("signals").select("*",{count:"exact",head:true}).gte("created_at",today+"T00:00:00"),supabase.from("waitlist").select("*",{count:"exact",head:true}),supabase.from("sweep_events").select("*").order("created_at",{ascending:false}),supabase.from("grants").select("*").order("created_at",{ascending:false}),supabase.from("signals").select("id,text,coherence,country,created_at").order("created_at",{ascending:false}).limit(20)]);const countries=new Set((sigAll.data||[]).map(s=>s.country).filter(Boolean)).size;const balance=sweeps.data&&sweeps.data.length>0?sweeps.data[0].grant_pool_balance:0;const totalSwept=(sweeps.data||[]).reduce((a,s)=>a+s.sweep_amount,0);const totalGranted=(grantsData.data||[]).filter(g=>g.status==="approved").reduce((a,g)=>a+(g.amount_approved||0),0);setStats({signals:sigAll.count||0,signalsToday:sigToday.count||0,waitlist:waitlist.count||0,countries});setPool({balance,totalSwept,totalGranted});setSweepEvents(sweeps.data||[]);setGrants(grantsData.data||[]);setSignals(recentSigs.data||[]);}
