...
const TRANSMISSIONS = [
  "The heart does not open when things get better.\nIt opens when you stop running from how they are.",
  "Coherence is not a state you achieve.\nIt is a state you stop preventing.",
  "The nervous system is not your enemy.\nIt is the most honest thing about you.",
  "You do not need to be fixed.\nYou need to be felt.",
  "The fractal is not a metaphor.\nEvery layer of the heart contains the whole of it.",
  "Resonance does not require agreement.\nIt requires presence.",
  "You are not a person having a spiritual experience.\nYou are awareness itself, remembering.",
  "The field does not reward those who try hardest.\nIt opens for those who stop fleeing.",
];
...
function useSimulatedCoherence() {
  ...
  useEffect(() => {
    const id = setInterval(() => {
      t.current += 0.006;
      const v = 0.48
        + Math.sin(t.current * 0.22) * 0.25
        + Math.sin(t.current * 0.55) * 0.1
        + Math.sin(t.current * 1.1) * 0.04;
...
function PlanetaryOrb({ coherence }: { coherence: number }) {
  ...
  return (
    <canvas
      ...
      width={800}
      height={800}
      style={{ position: "absolute", inset: 0, width: "100%", height: "100%", opacity: 0.75, pointerEvents: "none" }}
    />
  );
}
...
function CoherenceParticles({ coherence }: { coherence: number }) {
  ...
  return (
    <canvas
      ...
      width={1000}
      height={1000}
      style={{ position: "absolute", inset: "-15%", width: "130%", height: "130%", pointerEvents: "none" }}
    />
  );
}
...
function RippleCanvas({ triggered }: { triggered: boolean }) {
  ...
  return (
    <canvas
      ...
      width={800}
      height={800}
      style={{ position: "absolute", inset: "-22%", width: "144%", height: "144%", pointerEvents: "none", zIndex: 20 }}
    />
  );
}
...
function FieldDot({ delay = "0s" }: { delay?: string }) {
  return (
    <span
      ...
      style={{ display: "inline-block", width: "5px", height: "5px", borderRadius: "9999px", backgroundColor: "#FF6B3D", animationDuration: "2.5s", animationDelay: delay, flexShrink: 0 }}
    />
  );
}
...
export default function HomePage() {
  ...
  return (
    <div style={{ backgroundColor: "#090705", minHeight: "100vh", width: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", overflow: "hidden", position: "relative" }}>
      ...
      {/* Fixed void gradient — full screen depth */}
      <div
        aria-hidden="true"
        style={{ position: "fixed", inset: 0, pointerEvents: "none", zIndex: 0, background: "radial-gradient(ellipse 90% 85% at 50% 46%, rgba(150,55,10,0.24) 0%, rgba(90,28,5,0.12) 38%, rgba(9,7,5,0) 68%)" }}
      />

      {/* Planetary orb — fixed background layer with parallax */}
      <div
        aria-hidden="true"
        style={{
          position: "fixed",
          ...
          top: "50%",
          left: "50%",
          transform: `translate(calc(-50% + ${earthOffset.x}px), calc(-50% + ${earthOffset.y}px))`,
          pointerEvents: "none",
          zIndex: 1,
          opacity: loaded ? 1 : 0,
      ...
      {/* Main content column — full width, centred */}
      <div style={{ position: "relative", zIndex: 10, width: "100%", maxWidth: "100vw", display: "flex", flexDirection: "column", alignItems: "center", padding: "clamp(2rem, 5vh, 5rem) 1rem clamp(2rem, 4vh, 4rem)" }}>
        ...
        >
          <p style={{ fontFamily: "monospace", fontSize: "clamp(0.52rem, 1.1vw, 0.7rem)", letterSpacing: "0.42em", textTransform: "uppercase", color: `rgba(212,160,60,${0.4 + coherence * 0.35})`, marginBottom: "clamp(0.6rem, 1.5vh, 1.2rem)", transition: "color 2s ease" }}>
            A field for those willing to feel
          </p>
          <div style={{ position: "relative", display: "inline-block" }}>
            {/* Planetary aura behind wordmark */}
            <div
              aria-hidden="true"
              style={{ position: "absolute", inset: "-30px -60px", borderRadius: "9999px", background: `radial-gradient(ellipse 100% 100% at 50% 50%, rgba(200,100,14,${0.05 + coherence * 0.09}) 0%, transparent 65%)`, filter: "blur(24px)", pointerEvents: "none", transition: "background 2s ease" }}
            />
            <h1 style={{ fontFamily: "Georgia, serif", fontWeight: 300, lineHeight: 1.0, position: "relative", margin: 0 }}>
              <span style={{ display: "block", fontSize: "clamp(3rem, 10vw, 8rem)", color: `rgba(222,188,75,${0.72 + coherence * 0.22})`, transition: "color 2s ease", letterSpacing: "-0.01em" }}>
                Resonant
              </span>
              <span style={{ display: "block", fontSize: "clamp(3rem, 10vw, 8rem)", fontStyle: "italic", color: `rgba(193,68,14,${0.78 + coherence * 0.2})`, transition: "color 2s ease", marginTop: "-0.12em", letterSpacing: "-0.01em" }}>
                Earth
              </span>
            </h1>
          </div>
        ...
        {/* Heart composite — dominant, full presence */}
        <div
          style={{
            position: "relative",
        ...
        >
          ...
          {/* Multi-layer coherence glow rings */}
          {[1.7, 1.38, 1.15].map((scale, i) => (
            <div
              ...
              style={{
                ...
                inset: 0,
                borderRadius: "9999px",
                background: `radial-gradient(ellipse 65% 65% at 50% 50%, rgba(255,${95 + i * 22},35,${(0.045 + coherence * 0.09) / (i + 1)}) 0%, transparent 68%)`,
                transform: `scale(${scale + coherence * 0.18 + (hovered ? 0.12 : 0)})`,
          ...
          <div
            style={{
              position: "absolute",
              inset: 0,
              transform: `translate(${heartOffset.x}px, ${heartOffset.y}px)`,
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
            <FieldDot />
            <span style={{ color: `rgba(232,221,208,${0.22 + coherence * 0.22})`, fontFamily: "monospace", fontSize: "clamp(0.52rem, 1vw, 0.63rem)", letterSpacing: "0.32em", textTransform: "uppercase", transition: "color 1.5s ease" }}>
              field coherence: live
            </span>
            <FieldDot delay="0.9s" />
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: "0.85rem" }}>
            ...
            <span style={{ color: `rgba(212,136,74,${0.55 + coherence * 0.35})`, fontFamily: "monospace", fontSize: "0.72rem", width: "2.8rem", transition: "color 1.5s ease" }}>
              {coherencePct}%
            </span>
          </div>
          <p style={{ color: "rgba(232,221,208,0.13)", fontFamily: "monospace", fontSize: "clamp(0.5rem, 0.9vw, 0.58rem)" }}>
            hover the heart · presence dilates the field
          </p>
        </div>

        {/* Transmission quote — typewriter */}
        <div
          style={{
            minHeight: "clamp(5rem, 10vh, 7rem)",
            ...
            textAlign: "center",
            margin: "clamp(1.5rem, 4vh, 3rem) 0 clamp(1rem, 2.5vh, 2rem)",
            opacity: loaded ? 1 : 0,
        ...
        >
          <p style={{ fontFamily: "Georgia, serif", fontStyle: "italic", fontSize: "clamp(0.95rem, 2.4vw, 1.22rem)", lineHeight: 1.9, color: "rgba(232,221,208,0.58)", whiteSpace: "pre-line", opacity: txVisible ? 1 : 0, transition: "opacity 1.4s ease" }}>
            {displayed}
            {txVisible && <span style={{ opacity: 0.28, fontStyle: "normal" }}>▎</span>}
          </p>
        </div>
        ...
        >
          <button
            style={{ padding: "clamp(0.7rem, 1.5vh, 0.9rem) clamp(1.4rem, 3vw, 2.4rem)", border: `1px solid rgba(193,68,14,${0.28 + coherence * 0.32})`, backgroundColor: "transparent", color: "rgba(232,221,208,0.72)", fontFamily: "monospace", fontSize: "clamp(0.58rem, 1.1vw, 0.68rem)", letterSpacing: "0.24em", textTransform: "uppercase", cursor: "crosshair", transition: "all 0.4s ease", boxShadow: `0 0 22px rgba(193,68,14,${coherence * 0.14})` }}
            onMouseEnter={(e) => { const el = e.currentTarget; el.style.backgroundColor = "rgba(193,68,14,0.13)"; el.style.borderColor = "#C1440E"; el.style.color = "rgba(232,221,208,1)"; el.style.boxShadow = "0 0 35px rgba(193,68,14,0.35)"; }}
            onMouseLeave={(e) => { const el = e.currentTarget; el.style.backgroundColor = "transparent"; el.style.borderColor = `rgba(193,68,14,${0.28 + coherence * 0.32})`; el.style.color = "rgba(232,221,208,0.72)"; el.style.boxShadow = `0 0 22px rgba(193,68,14,${coherence * 0.14})`; }}
          >
            Enter the Field
          </button>
          <button
            style={{ padding: "clamp(0.7rem, 1.5vh, 0.9rem) clamp(1.4rem, 3vw, 2.4rem)", backgroundColor: "transparent", border: "none", color: "rgba(232,221,208,0.28)", fontFamily: "monospace", fontSize: "clamp(0.58rem, 1.1vw, 0.68rem)", letterSpacing: "0.24em", textTransform: "uppercase", cursor: "crosshair", transition: "color 0.4s ease" }}
            onMouseEnter={(e) => { e.currentTarget.style.color = "rgba(232,221,208,0.62)"; }}
        ...
        <footer style={{ width: "100%", borderTop: "1px solid rgba(193,68,14,0.09)", padding: "clamp(1.5rem, 4vh, 2.8rem) 1rem", textAlign: "center" }}>
          <div style={{ maxWidth: "min(620px, 90vw)", margin: "0 auto", display: "flex", flexDirection: "column", gap: "clamp(0.6rem, 1.5vh, 1rem)" }}>
            <p style={{ color: `rgba(212,136,74,${0.3 + coherence * 0.2})`, fontFamily: "monospace", fontSize: "clamp(0.52rem, 0.95vw, 0.62rem)", letterSpacing: "0.32em", textTransform: "uppercase", transition: "color 2s ease" }}>
              Resonant Earth · Est. 2026
            </p>
            <p style={{ color: "rgba(232,221,208,0.26)", fontFamily: "monospace", fontSize: "clamp(0.52rem, 0.95vw, 0.62rem)", lineHeight: 2.1 }}>
              ...
              Nothing here will take your pain away —<br />
              it will only help you stop running from it long enough<br />
              to discover what it is trying to show you.
            </p>
            <p style={{ color: "rgba(232,221,208,0.14)", fontFamily: "monospace", fontSize: "clamp(0.48rem, 0.88vw, 0.58rem)", lineHeight: 1.95 }}>
              Resonant Earth does not diagnose, treat, or cure.<br />
              It does not promise transformation, healing, or relief.<br />
              It offers only a field — and the invitation to stop leaving it.
            </p>
            <p style={{ color: "rgba(232,221,208,0.09)", fontFamily: "monospace", fontSize: "clamp(0.46rem, 0.82vw, 0.55rem)", paddingTop: "0.4rem" }}>
              The chest blooming open is not a metaphor. It is an instruction.
            </p>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "0.5rem", paddingTop: "0.2rem" }}>
              <FieldDot />
              <span style={{ color: "rgba(232,221,208,0.1)", fontFamily: "monospace", fontSize: "0.52rem" }}>field coherence: live</span>
              <FieldDot delay="0.5s" />
            </div>
          </div>
        </footer>
